		...
		Context("when the -f argument is given", func() {
			...
			Context("when the plugin is invalid", func() {
				...
				BeforeEach(func() {
					...
					fakeActor.ValidatePluginReturns(configv3.Plugin{}, returnedErr)
				})
			...
			Context("when the plugin is not already installed", func() {
				...
				BeforeEach(func() {
					...
					fakeActor.ValidatePluginReturns(plugin, nil)
				})
				...
				It("installs the plugin", func() {
					...
					Expect(fakeActor.ValidatePluginCallCount()).To(Equal(1))
					_, _, path := fakeActor.ValidatePluginArgsForCall(0)
			...
			Context("when the plugin is already installed", func() {
				...
				BeforeEach(func() {
					...
					fakeActor.ValidatePluginReturns(
						plugin,
						pluginaction.PluginAlreadyInstalledError{Name: "some-plugin", Version: "1.0.0"})
				})
		...
		Context("when the -f argument is not given (user is prompted for confirmation)", func() {
			...
			Context("when the user chooses yes", func() {
				...
				Context("when the plugin is already installed", func() {
					...
					BeforeEach(func() {
						returnedErr = pluginaction.PluginAlreadyInstalledError{Name: "some-plugin", Version: "1.0.0"}
						fakeActor.ValidatePluginReturns(configv3.Plugin{}, returnedErr)
					})
