...
var _ = Describe("Job", func() {
  ...
  "error_code": "CF-OrganizationNotFound"
	...
	Describe("UploadApplicationBits", func() {
		BeforeEach(func() {
			client = NewTestClient()
		})
		...
		Context("when the upload is successful", func() {
			...
			It("returns the created job and warnings", func() {
				job, warnings, err := client.UploadApplicationBits("some-app-guid", resources, reader)
				Expect(err).NotTo(HaveOccurred())
		...
		Context("when the CC returns an error", func() {
			...
			It("returns the error", func() {
				_, warnings, err := client.UploadApplicationBits("some-app-guid", []Resource{}, bytes.NewReader(nil))
		...
		Context("when passed a nil resources", func() {
			It("returns a NilObjectError", func() {
				_, _, err := client.UploadApplicationBits("some-app-guid", nil, bytes.NewReader(nil))
		...
		Context("when passed a nil reader", func() {
			It("returns a NilObjectError", func() {
				_, _, err := client.UploadApplicationBits("some-app-guid", []Resource{}, nil)
		...
		Context("when an error is returned from the new resources reader", func() {
			...
			It("returns the error", func() {
				_, _, err := client.UploadApplicationBits("some-app-guid", []Resource{}, fakeReader)
